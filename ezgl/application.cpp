...
void application::register_default_buttons_callbacks(ezgl::application *application)
{
  ...
  // Connect press_find function to the SelectMap combobox
  GObject *map_selector = application->get_object("SelectMap");
  g_signal_connect(map_selector, "changed", G_CALLBACK(change_map), application);
  
  // Connect toggle_walk function to the ToggleWalk checkbox
  GObject *walk_toggle = application->get_object("ToggleWalk");
  g_signal_connect(walk_toggle, "toggled", G_CALLBACK(toggle_walk), application);
}
...
//update the text in the IntersectionInfo GTKlabel
void application::update_info(std::string const &message)
{
  GtkLabel *info = (GtkLabel *)get_object("IntersectionInfo");

  gtk_label_set_text(info, message.c_str());
}

//Gets the text from the search bar for the first street
const std::string application::get_first_street(){
   GtkEntry *search_bar = (GtkEntry *)get_object("StreetEntry1");
   //get the inputted text from the search bar as a char array
   const gchar* text = gtk_entry_get_text(search_bar);
   //convert text to string
   const std::string stringText(text);
   
   return stringText;
}

//Gets the text from the search bar for the second street
const std::string application::get_second_street(){
   GtkEntry *search_bar = (GtkEntry *)get_object("StreetEntry2");
   //get the inputted text from the search bar as a char array
   const gchar* text = gtk_entry_get_text(search_bar);
   //convert text to string
   const std::string stringText(text);
...
//Gets the text from the search bar for the walk time
const std::string application::get_walk_time(){
   GtkEntry *search_bar = (GtkEntry *)get_object("WalkTime");
   //get the inputted text from the search bar as a char array
   const gchar* text = gtk_entry_get_text(search_bar);
   //convert text to string
   const std::string stringText(text);
...
//Gets the text from the search bar for the walk speed
const std::string application::get_walk_speed(){
   GtkEntry *search_bar = (GtkEntry *)get_object("WalkSpeed");
   //get the inputted text from the search bar as a char array
   const gchar* text = gtk_entry_get_text(search_bar);
   //convert text to string
   const std::string stringText(text);
...
//Changes visibility of walk search bars
void application::change_visibility(){
   GtkWidget *walk_box = (GtkWidget *)get_object("WalkBox");
   
   gtk_widget_set_visible(walk_box, !gtk_widget_get_visible(walk_box));
}

//Gets the map ID from the drop down menu when you click on an option
const std::string application::get_map_name(){
   GtkComboBox *map_selector = (GtkComboBox *)get_object("SelectMap");
   //gets the id of the selected option in the combobox, the IDs corresponds to the path of each map
   const gchar* text = gtk_combo_box_get_active_id(map_selector);
   //convert text to string
   const std::string stringText(text);
